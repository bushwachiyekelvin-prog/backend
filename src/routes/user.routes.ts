...
export async function userRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    {
      schema: {
        response: { 200: UserModel.SignUpResponseSchema, 400: UserModel.ErrorResponseSchema },
        tags: ['user'],
      },
      config: { rawBody: true },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return reply.code(400).send({ error: 'Invalid event type', code: 'INVALID_EVENT_TYPE' });
      } catch (err) {
        logger.error('Unexpected error while handling sign-up:', err);
        return reply.code(400).send({ error: 'Unexpected error while handling sign-up', code: 'UNEXPECTED_ERROR' });
      }
    },
  );
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        return reply.code(500).send({ error: 'Failed to verify OTP', code: 'OTP_VERIFICATION_FAILED' });
      }
    },
  );
