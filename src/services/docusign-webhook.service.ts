import { db } from "../db";
import { offerLetters, loanApplications, users } from "../db/schema";
...
export class DocuSignWebhookService {
  static async handleStatusUpdate(event: any): Promise<void> {
    ...
    try {
      ...
      // Update offer letter
      logger.info(
        `Updating offer letter ${offerLetter.id}: status ${offerLetter.status} -> ${newOfferLetterStatus}, docuSignStatus ${offerLetter.docuSignStatus} -> ${newDocuSignStatus}`
      );
      ...
      try {
        logger.info(
          `Creating audit trail for offer letter ${offerLetter.id} with status ${status}, createdBy: ${offerLetter.createdBy}`
        );

        // Get the user who created the offer letter for audit trail
        const user = offerLetter.createdBy
        ...
        if (user) {
          ...
          logger.warn(
            `No user found for offer letter ${offerLetter.id}, createdBy: ${offerLetter.createdBy}`
          );
        }
      } catch (auditError) {
      ...
      if (status === "completed" || status === "declined") {
        const newLoanStatus =
          status === "completed" ? "offer_letter_signed" : "offer_letter_declined";

        // Update loan application status directly (without triggering automation)
        if (newLoanStatus) {
          try {
            logger.info(
              `Updating loan application ${offerLetter.loanApplicationId} status to ${newLoanStatus} via webhook`
            );

            // Get the user's Clerk ID for the foreign key constraint
            let creatorClerkId = "system";
            ...
            logger.info(
              `Successfully updated loan application ${offerLetter.loanApplicationId} status to ${newLoanStatus} via webhook`
            );
          } catch (statusError) {
      ...
      logger.info(
        `Updated offer letter ${offerLetter.id} status to ${newOfferLetterStatus} based on DocuSign event ${status}`
      );

      // Invalidate cache for the loan application
      try {
        logger.info("Invalidating cache for updated loan application", {
          loanApplicationId: offerLetter.loanApplicationId,
          envelopeId,
        });
        await CachingService.invalidateLoanApplication(offerLetter.loanApplicationId);
        logger.info("Cache invalidation completed for loan application", {
          loanApplicationId: offerLetter.loanApplicationId,
        });
      } catch (cacheError) {
        logger.error("Error invalidating cache after DocuSign status update", {
          loanApplicationId: offerLetter.loanApplicationId,
          error: cacheError instanceof Error ? cacheError.message : cacheError,
        });
        // Don't throw - cache invalidation failure shouldn't prevent webhook processing
      }
    } catch (error) {
