...
export async function loanProductsRoutes(fastify) {
    // CREATE loan product
    fastify.post("/", {
        schema: {
        ...
        try {
            ...
            const result = await LoanProductsService.create(userId, request.body);
    ...
    fastify.patch("/:id", {
        ...
        try {
            ...
            const { id } = request.params || {};
            const result = await LoanProductsService.update(userId, id, request.body);
            return reply.send(result);
        }
        catch (error) {
