...
export class Business {
     ...
     */
    static async register(clerkId, payload) {
        try {
            if (!clerkId) {
            ...
            const values = {
                ...
                ownershipPercentage: typeof payload.ownershipPercentage === "number"
            ...
            if (error?.status)
                throw error;
            throw httpError(500, "[BUSINESS_REGISTER_ERROR] An error occurred while registering the business");
        }
    }
    /**
     ...
     */
    static async edit(clerkId, businessId, payload) {
        try {
            if (!clerkId) {
                throw httpError(401, "[UNAUTHORIZED] Missing user context");
            }
            ...
            if (payload.zipCode !== undefined)
                updates.zipCode = payload.zipCode ?? null;
            if (payload.address2 !== undefined)
                updates.address2 = payload.address2 ?? null;
            if (payload.yearOfIncorporation !== undefined)
                updates.yearOfIncorporation = String(payload.yearOfIncorporation);
            if (payload.isOwned !== undefined)
                updates.isOwned = payload.isOwned;
            ...
            if (payload.loanStatus !== undefined)
                updates.loanStatus = payload.loanStatus ?? null;
     ...
     * @param clerkId Clerk user id, used to resolve internal users.id
     */
    static async listByUser(clerkId) {
        try {
            if (!clerkId) {
                throw httpError(401, "[UNAUTHORIZED] Missing user context");
            }
            ...
            const data = rows.map((r) => ({
                ...r,
                avgMonthlyTurnover: r.avgMonthlyTurnover !== null && r.avgMonthlyTurnover !== undefined
                    ? Number(r.avgMonthlyTurnover)
                    : null,
                avgYearlyTurnover: r.avgYearlyTurnover !== null && r.avgYearlyTurnover !== undefined
