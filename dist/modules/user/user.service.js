...
export class User {
    static async signUp(userPayload) {
        try {
            // Ensure dob is a Date object for the DB layer
            const values = {
                ...userPayload,
                dob: typeof userPayload.dob === "string"
     ...
     */
    static async sendPhoneVerificationOtp(clerkId) {
        try {
            // Get user details
            const user = await db.query.users.findFirst({
     ...
     * @returns Success status
     */
    static async verifyPhoneOtp(clerkId, otp) {
        try {
            // Get user details
            const user = await db.query.users.findFirst({
                where: eq(users.clerkId, clerkId),
            ...
            // Validate OTP
            const isValid = OtpUtils.validateOtp(otp, user.phoneVerificationCode || null, user.phoneVerificationExpiry || null);
            if (!isValid) {
                return {
                    success: false,
     ...
     * @returns Success status
     */
    static async resendPhoneVerificationOtp(clerkId) {
     ...
     * */
    static async updatePhoneNumber(clerkId, phoneNumber) {
        try {
            const user = await db.query.users.findFirst({
                where: eq(users.clerkId, clerkId),
            });
     ...
     */
    static async updateUserAndDocuments(clerkId, payload) {
        try {
            ...
            await db.transaction(async (tx) => {
                ...
                if (Array.isArray(payload.documents) && payload.documents.length > 0) {
                    ...
                    await tx.insert(personalDocuments).values(payload.documents.map((doc) => ({
                        userId: user.id,
                        docType: doc.docType,
                        docUrl: doc.docUrl,
                    })));
     ...
     */
    static async editProfile(clerkId, payload) {
        try {
            // Resolve internal user
            const user = await db.query.users.findFirst({
                where: eq(users.clerkId, clerkId),
     ...
     */
    static async updateEmail(clerkId, email) {
        try {
            // Resolve target user
            const user = await db.query.users.findFirst({
                where: eq(users.clerkId, clerkId),
